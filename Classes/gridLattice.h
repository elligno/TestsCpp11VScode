...
{
    /**
     * @brief class GridLattice1D describes the domain the PDE 
	 *        will be applied to
     */
    class gridLattice1D 
    {
    public:
         ...
         * @param aInitStr 
         */
		gridLattice1D( const std::string& aInitStr)
        : m_xmin{},
          m_xmax{},
          m_division{}
        {
        ...
        is.ignore(1, ' '); is.ignore(1, '[');
			...
			double r;  is >> r;  m_xmin = r;
			is.ignore (1, ',');

			double s;  is >> s;  m_xmax = s;
			is.ignore(1, ']');
...
//		}
    ...
    int getDivisions() const { return m_division;}
		...
		friend std::ostream& operator<< ( std::ostream& os, gridLattice1D& g)
        {
            auto dimensions = 1;
